class MetalPriceHistory < ApplicationRecord
  include CountsHelper

  # Validations
  validates :metal, :karat, :price_per_gram, :recorded_at, presence: true
  ...
  validates :price_per_gram, numericality: { greater_than_or_equal_to: 0.0 }
  validates :karat, numericality: { greater_than_or_equal_to: 0.0 }

  # Callbacks
  before_validation { self.price_per_gram = self.price_per_gram.to_f.round(2) if self.price_per_gram }
  ...
  def self.latest_price(metal, karat)
    where(metal: metal, karat: karat).order(recorded_at: :desc).first
  end
